...
type RegisterProps = {
  ...
  setRegErrors: React.Dispatch<React.SetStateAction<Partial<Record<keyof RegisterRequestDto, string>>>>;
  prevStep: () => void;
  nextStep: () => void;
            ...
            return (
              <li key={label} className="flex-1">
                <div
                  ...
                  ${isActive ? 'border-primary text-primary shadow-sm' : isDone ? 'border-green-500 text-green-700' : 'border-gray-200 text-gray-500'}`}
                  aria-current={isActive ? 'step' : undefined}
                >
                  <span
                    className={`grid h-6 w-6 place-items-center rounded-full text-xs font-semibold
                    ${isActive ? 'bg-primary text-white' : isDone ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'}`}
                  >
                    {idx + 1}
                  </span>
          ...
          <div className="flex w-full flex-col gap-4 xl:px-20 2xl:px-50">
            <InputField
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.firstName) setRegErrors((p) => ({ ...p, firstName: undefined }));
              }}
            />
            <InputField
              id="reg-lastName"
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.lastName) setRegErrors((p) => ({ ...p, lastName: undefined }));
              }}
            />
            <InputField
              id="reg-email"
          ...
          <div className="flex w-full flex-col gap-4 xl:px-20 2xl:px-50">
            <InputField
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.specialization) setRegErrors((p) => ({ ...p, specialization: undefined }));
              }}
            />
            <InputField
              id="reg-facilityName"
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.facilityName) setRegErrors((p) => ({ ...p, facilityName: undefined }));
              }}
            />
            <InputField
              id="reg-facilityCity"
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.facilityCity) setRegErrors((p) => ({ ...p, facilityCity: undefined }));
              }}
            />
          </div>
        )}
          ...
          <div className="flex w-full flex-col gap-4 xl:px-20 2xl:px-50">
            ...
            <InputField
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (regErrors.confirmPassword) setRegErrors((p) => ({ ...p, confirmPassword: undefined }));
              }}
            />
          </div>
        )}
...
const LoginPage = () => {
  ...
  // LOGIN
  const [userLogInInfo, setUserLogInInfo] = useState<LoginRequestDto>(initialLogin);
  ...
  const [regErrors, setRegErrors] = useState<Partial<Record<keyof RegisterRequestDto, string>>>({});

  const validateLogin = () => {
    const errs: typeof loginErrors = {};
    if (!userLogInInfo.email.trim()) errs.email = t('errors.emailNotEmpty');
    if (!userLogInInfo.password.trim()) errs.password = t('errors.passwordNotEmpty');
    setLoginErrors(errs);
  ...
  const validateStep = (s: 0 | 1 | 2) => {
    ...
    if (s === 0) {
      if (!reg.firstName.trim()) errs.firstName = t('errors.fieldRequired') || 'Wymagane';
      if (!reg.lastName.trim()) errs.lastName = t('errors.fieldRequired') || 'Wymagane';
      if (!reg.email.trim()) errs.email = t('errors.emailNotEmpty') || 'Podaj email';
    }
    if (s === 1) {
      if (!reg.specialization.trim()) errs.specialization = t('errors.fieldRequired') || 'Wymagane';
      if (!reg.facilityName.trim()) errs.facilityName = t('errors.fieldRequired') || 'Wymagane';
      if (!reg.facilityCity.trim()) errs.facilityCity = t('errors.fieldRequired') || 'Wymagane';
    }
    if (s === 2) {
      if (!reg.password.trim()) errs.password = t('passwordNotEmpty') || 'Podaj hasło';
      if (reg.password !== reg.confirmPassword) {
        errs.confirmPassword = t('errors.passwordsMustMatch') || 'Hasła muszą się zgadzać';
      }
    }
    setRegErrors(errs);
    return Object.keys(errs).length === 0;
          ...
          {mode === 'login' ? (
            <div className="flex h-full w-full flex-col justify-between">
              ...
              <div className="mx-auto flex max-w-sm items-center gap-x-4">
                <img src="/logo.svg" alt="Logo" className="h-16 w-16 object-cover" />
                <div className="text-primary-accent color text-3xl font-bold">Remisio</div>
              </div>

              {/* Body */}
              <div className="flex w-full flex-col items-center justify-center">
                <span className="text-primary-accent mb-2 text-3xl font-bold">{t('welcome')}</span>
                <span className="text-primary-accent mb-6">{t('subtitle')}</span>

                <form className="flex w-full flex-col gap-4 xl:px-20 2xl:px-50" onSubmit={handleLoginSubmit}>
                  <InputField
                    id="login-email"
                    label={t('email')}
                    type="email"
                  ...
                  />
                  <Button className="font-bold" type="submit" isLoading={login.isPending}>
                    {t('login')}
                  </Button>
                </form>
