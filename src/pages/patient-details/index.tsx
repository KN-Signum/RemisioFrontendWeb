...
const PatientDetailsPage = () => {
    ...
    // Find the patient and ensure age is computed if missing
    const rawPatient = mockPatients.find((p) => p.id === id);

    // Create a copy of the patient with age computed from date_of_birth if needed
    const patient = rawPatient
        ? {
            ...rawPatient,
            // If age is missing but date_of_birth exists, calculate age
            age: rawPatient.age ?? (rawPatient.date_of_birth
                ? Math.floor((new Date().getTime() - new Date(rawPatient.date_of_birth).getTime()) / (365.25 * 24 * 60 * 60 * 1000))
                : 0)
        }
        : undefined;
