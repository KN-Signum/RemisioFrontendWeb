...
const updateSymptom = http.put(
    ...
    async ({ params, request }) => {
        ...
        if (!existingSymptom) {
            return HttpResponse.json({ status: 404, content: { message: 'Symptom not found' } });
        }
...
const deleteSymptom = http.delete(
    ...
    ({ params }) => {
        ...
        return HttpResponse.json({ status: 200, content: { message: 'Symptom deleted successfully' } });
    },
);

export const handlers = [getSymptomsByPatientId, createSymptom, updateSymptom, deleteSymptom];