...
export const handlers = [
    ...
    http.post(`${API_URL}/api/patients/:patientId/surveys/crohn`, async ({ params, request }) => {
        ...
        const extraintestinalManifestations = data?.extraintestinal_manifestations ?? 0;
        const abdominalMass = data?.abdominal_mass ?? 0;
        const hematocrit = data?.hematocrit ?? 0;
        const weightLoss = data?.weight_loss ?? 0;

        const totalScore = data?.total_score ?? calculateCrohnTotalScore(
            abdominalPain,
            stools,
            generalWellbeing,
    ...
    http.get(`${API_URL}/api/patients/:patientId/surveys/crohn/latest`, ({ params }) => {
        const { patientId } = params;
        ...
        return HttpResponse.json(
            {
                message: latestSurvey ? 'Latest Crohn\'s survey retrieved successfully' : 'No Crohn\'s surveys found for this patient',
    ...
    http.post(`${API_URL}/api/patients/:patientId/surveys/uc`, async ({ params, request }) => {
        const { patientId } = params;
        const data = await request.json() as any;
        ...
        const totalScore = data?.total_score ?? calculateUcTotalScore(
            stoolFrequency,
            rectalBleeding,
            physicianGlobal
        );
    ...
    http.get(`${API_URL}/api/patients/:patientId/surveys/uc/latest`, ({ params }) => {
        const { patientId } = params;
        ...
        return HttpResponse.json(
            {
                message: latestSurvey ? 'Latest UC survey retrieved successfully' : 'No UC surveys found for this patient',
                content: latestSurvey
            },
            { status: 200 }
        );
