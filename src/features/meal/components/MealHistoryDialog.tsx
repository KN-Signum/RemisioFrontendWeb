...
export const MealHistoryDialog = ({ patientId, iconOnly = false, className = '' }: MealHistoryDialogProps) => {
                    ...
                    >
                        ...
                        <div className="mt-4 flex-1 overflow-y-auto">
                            {isLoading ? (
                                ...
                                <div className="flex h-40 items-center justify-center">
                                    <p className="text-lg">{t('meal.history.no_meals', 'No meals recorded')}</p>
                                </div>
                            ) : (
                                    ...
                                    {Object.keys(groupedMeals).map((date) => (
                                        <div key={date} className="mb-4">
                                            ...
                                            {groupedMeals[date].map((meal) => (
                                                <div key={meal.id} className="mb-3 rounded-sm bg-background/10 p-4">
                                                    ...
                                                    {meal.meal_notes && (
                                                        <div className="mt-2 border-t border-gray-600 pt-2">
                                                            <p className="text-sm">
                                                                <span className="font-semibold">{t('meal.notes', 'Notes')}:</span> {meal.meal_notes}
                                                            </p>
                                                        </div>
                                                    )}
                                                </div>
