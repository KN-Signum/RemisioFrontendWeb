...
export const SymptomHistoryDialog = ({ patientId, iconOnly = false, className = '' }: SymptomHistoryDialogProps) => {
    ...
    const sortedSymptoms = symptoms ? [...symptoms].sort((a, b) => {
                    ...
                    >
                        ...
                        <div className="mt-4 flex-1 overflow-y-auto">
                            {isLoading ? (
                                ...
                                <div className="flex h-40 items-center justify-center">
                                    <p className="text-lg">{t('symptoms.history.no_symptoms', 'No symptoms recorded')}</p>
                                </div>
                            ) : (
                                <div className="flex flex-col gap-4">
                                    {sortedSymptoms.map((symptom: SymptomDto) => (
                                        <div key={symptom.id} className="rounded-sm bg-background/10 p-4">
                                            <div className="mb-2 flex items-center justify-between">
                                                <h3 className="text-lg font-semibold">
                                                    {symptom.created_at ? formatDateDisplay(new Date(symptom.created_at)) : 'Unknown date'}
                                                </h3>
                                                <div className="flex items-center gap-2">
                                                    <span className="font-semibold">{t('symptoms.type', 'Type')}:</span>
                                                    <span>{symptom.symptom_type}</span>
                                                    <span className={`ml-2 font-bold ${getPainLevelColorClass(symptom.pain_level)}`}>
                                                        {symptom.pain_level}
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-sm">
                                                <div>
                                                    <p>
                                                        <span className="font-semibold">{t('symptoms.duration', 'Duration')}:</span> {symptom.duration}
                                                    </p>
                                                </div>
                                                <div>
                                                    <p>
                                                        <span className="font-semibold">{t('symptoms.pain_level', 'Pain Level')}:</span> {symptom.pain_level}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            {symptom.symptom_description && (
                                                <div className="mt-2 border-t border-gray-600 pt-2">
                                                    <p className="text-sm">
                                                        <span className="font-semibold">{t('symptoms.description', 'Description')}:</span> {symptom.symptom_description}
                                                    </p>
                                                </div>
                                            )}
                                        </div>
