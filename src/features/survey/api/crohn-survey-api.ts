 ...
 */
export const createCrohnSurvey = async (data: CreateCrohnSurveyDto): Promise<CrohnSurveyDto> => {
    ...
    const response = await apiClient.post(`/api/patients/${data.patient_id}/surveys/crohn`, {
        ...data,
 ...
 */
export const getPatientCrohnSurveys = async (patientId: string): Promise<GetPatientSurveysDto<CrohnSurveyDto>> => {
    const response = await apiClient.get(`/api/patients/${patientId}/surveys/crohn`);
    console.log('[API-CLIENT] ←', response.data);
    return response.data.content;
};
 ...
 */
export const getLatestPatientCrohnSurvey = async (patientId: string): Promise<CrohnSurveyDto | null> => {
    const response = await apiClient.get(`/api/patients/${patientId}/surveys/crohn/latest`);
    console.log('[API-CLIENT] ←', response.data);
    return response.data.content;
};
