 ...
 */
export const createUcSurvey = async (data: CreateUcSurveyDto): Promise<UcSurveyDto> => {
    ...
    const response = await apiClient.post(`/api/patients/${data.patient_id}/surveys/uc`, {
        ...data,
 ...
 */
export const getPatientUcSurveys = async (patientId: string): Promise<GetPatientSurveysDto<UcSurveyDto>> => {
 ...
 */
export const getLatestPatientUcSurvey = async (patientId: string): Promise<UcSurveyDto | null> => {
    const response = await apiClient.get(`/api/patients/${patientId}/surveys/uc/latest`);
    console.log('[API-CLIENT] ←', response.data);
    return response.data.content;
};
